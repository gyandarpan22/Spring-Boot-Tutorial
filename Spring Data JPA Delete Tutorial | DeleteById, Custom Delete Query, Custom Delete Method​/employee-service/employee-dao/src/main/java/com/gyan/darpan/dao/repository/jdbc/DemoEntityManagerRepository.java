...
//                criteriaBuilder.construct(DepartmentCount.class, root.get("department"), countExpression);
    ...
    public Employee updateEmployee(Employee employee) {
        String query = "Update Employee emp set emp.employeeName=:employeeName , emp.age=:age , emp.department=:department where emp.employeeId=:employeeId";


        int updateCount = entityManager.createQuery(query)
    ...
    public Employee findByEmployeeIdUsingSql(long employeeId) {
        String sql = "select *from employee where employee_id=?";

        return (Employee) entityManager.createNativeQuery(sql, Employee.class)
                .setParameter(1, employeeId)
                .getSingleResult();
    }

    @Transactional
    public Employee insertEmployeeUsingSql(Employee employee) {
        String query = "insert into employee(employee_name,age,joining_date,department) values (?,?,?,?)";

        entityManager.createNativeQuery(query)
                .setParameter(1, employee.getEmployeeName())
                .setParameter(2, employee.getAge())
                .setParameter(3, employee.getJoiningDate())
                .setParameter(4, employee.getDepartment())
                .executeUpdate();

        query = "Select last_insert_id()";

        Number employeeId = (Number) entityManager.createNativeQuery(query)
                .getSingleResult();

        employee.setEmployeeId(employeeId.longValue());
